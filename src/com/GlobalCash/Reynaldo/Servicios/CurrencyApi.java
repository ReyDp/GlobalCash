...
public class CurrencyApi {
    ...
    public Conversion convertCurrency(Currency from, Currency to, double amount) {
        String urlSearch = String.format("%s%s/pair/%s/%s/%s", API_URL, API_KEY, from.toString(), to.toString(), amount);

        HttpClient client = HttpClient.newHttpClient(); // Crear el cliente sin try-with-resources
